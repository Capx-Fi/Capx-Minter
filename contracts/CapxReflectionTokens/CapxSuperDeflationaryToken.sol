...
contract CapxSuperDeflationaryToken is Ownable, IERC20, IERC20Metadata {
    ...
    uint256 internal constant _MAX = ~uint256(0);

    uint256 internal _tTotal;
    uint256 internal _rTotal;
    uint256 internal _tFeeTotal;

    bool public _taxEnabled;
    ...
    uint256 internal _prevTaxFee;
    uint256 internal _prevLiquidityFee;
    uint256 internal _prevBurnFee;
    uint256 internal _previousmarketingFee;

    IRouter02 public  v2Router;
    ...
    /**
    * @param address_ address parameters:
    ...
    * @param parameters_ address parameters:
    ...
    */
    function initializer (
        ...
        _taxFee = parameters_[0];
        _burnFee = parameters_[1];
        _liquidityFee = parameters_[2];
        ...
        _transferOwnership(address_[0]);

        // Transfer the Initial Supply.
        _rOwned[address_[0]] = _rTotal;
        emit Transfer(address(0), address_[0], _tTotal);
    }
    ...
    function _beforeTransfer(address from) internal {
        ...
        bool overMinTokenBalance = contractTokenBalance >= autoLPThreshold;
        if (
            overMinTokenBalance &&
            !inSwapAndLiquify &&
            from != v2Pair &&
            ...
            contractTokenBalance = autoLPThreshold;
            //add liquidity
    ...
    function _getTValues(uint256 tAmount) internal view returns (uint256, uint256, uint256) {
        uint256 tFee = tAmount.div(10000).mul(_taxFee);
        uint256 tLiquidity = tAmount.div(10000).mul(_liquidityFee);
    ...
    function takeBurn(address sender, uint256 tTransferAmount, uint256 rTransferAmount, uint256 tAmount) internal
    ...
    function takeMarketing(address sender, uint256 tTransferAmount, uint256 rTransferAmount, uint256 tAmount) internal
    returns (uint256, uint256)
    {
    ...
    function setRouterAddress(address newRouter) public onlyOwner() {
        ...
        v2Pair = IFactory(_newRouter.factory()).createPair(address(this), _newRouter.WETH());
        v2Router = _newRouter;
    }

    function setAutoLPThreshold(uint256 threshold) external onlyOwner() {
